...
if page == "Candidate Interview":
    ...
    if matched_schedule:
        st.success(f"Welcome {matched_schedule['candidate_name']} — Role: {matched_schedule['role']} | Round: {matched_schedule['round_name']}")

    if "agent" not in st.session_state:
        st.subheader("Candidate Details")
        if matched_schedule:
            candidate_name = matched_schedule["candidate_name"]
            job_title = matched_schedule["role"]
            technical_skills = ", ".join(matched_schedule.get("technical_skills", []))
            soft_skills = ", ".join(matched_schedule.get("soft_skills", []))
            experience = matched_schedule.get("experience_required", "")
            difficulty = matched_schedule.get("difficulty", "Mid")
            round_name = matched_schedule["round_name"]
            st.info(f"Auto-filled — Candidate: {candidate_name} | Role: {job_title} | Round: {round_name}")
        else:
            candidate_name = st.text_input("Candidate Name")
            st.subheader("Job Details")
            job_title = st.text_input("Job Title", placeholder="e.g. Data Scientist, Frontend Developer, HR Manager...")
            technical_skills = st.text_input("Technical Skills (comma separated)", placeholder="e.g. Python, SQL, TensorFlow")
        ...
        extra_q = st.text_area("Extra must-ask questions (one per line)", placeholder="e.g. How do you handle tight deadlines?")
        if extra_q:
        ...
        st.caption(f"Candidate: {st.session_state.candidate_name} | Role: {st.session_state.jd_data['job_title']} | Round: {round_name}")
    ...
    if not all_candidates:
        ...
        for candidate in reversed(filtered):
            ...
            with st.expander(f"{badge} {candidate['candidate_name']} — {candidate['role']} — Rounds: {rounds_done} — Score: {score}/10"):
                col1, col2 = st.columns(2)
                with col1:
                    ...
                    st.markdown(f"**Rounds Completed:** {rounds_done}")
                with col2:
                ...
                for rnd in rounds:
                    ...
                    col1, col2 = st.columns(2)
                    with col1:
                        st.download_button(label=f"Download {rnd['round_name']} TXT", data=rnd["report"], file_name=f"{candidate['candidate_name']}_{candidate['role']}_{rnd['round_name']}_report.txt", mime="text/plain", key=f"txt_{candidate['candidate_name']}_{rnd['round_name']}_{rnd['date']}")
                    with col2:
                        pdf_path = generate_pdf(candidate["candidate_name"], candidate["role"], rnd["date"], rnd["report"], rnd["round_name"])
                        with open(pdf_path, "rb") as f:
                            st.download_button(label=f"Download {rnd['round_name']} PDF", data=f, file_name=f"{candidate['candidate_name']}_{candidate['role']}_{rnd['round_name']}_report.pdf", mime="application/pdf", key=f"pdf_{candidate['candidate_name']}_{rnd['round_name']}_{rnd['date']}")
    ...
    question_bank = load_question_bank()
    st.subheader("Add Questions for Any Role")
    new_role = st.text_input("Role Name", placeholder="e.g. Data Scientist, HR Manager")
    new_questions = st.text_area("Questions (one per line)", placeholder="e.g. Explain a project you built from scratch")
    ...
    with col1:
        ...
        sch_role = st.text_input("Job Title", placeholder="e.g. Backend Developer")
        sch_experience = st.text_input("Experience Required", placeholder="e.g. 2 years")
    with col2:
        sch_technical = st.text_input("Technical Skills (comma separated)", placeholder="e.g. Python, SQL")
        sch_soft = st.text_input("Soft Skills (comma separated)", placeholder="e.g. communication, teamwork")
    ...
    if st.button("Generate Interview Link"):
        if sch_candidate and sch_role:
            ...
            st.code(f"?token={token}", language=None)
            st.info("Share with candidate — add to your app URL: your-app.streamlit.app/?token=" + token)
            st.rerun()
        else:
    ...
    if not schedules:
        ...
        for s in reversed(schedules):
            ...
            with st.expander(f"{status} — {s['candidate_name']} — {s['role']} — {s['round_name']} — {s['created']}"):
                ...
                st.code(f"?token={s['token']}", language=None)
    ...
    if not all_candidates:
        ...
        if not all_rounds:
            ...
            if all_scores:
                fig = px.histogram(x=all_scores, nbins=10, labels={"x": "Overall Score", "y": "Count"}, color_discrete_sequence=["#4F8BF9"])
